...
input int      InpFastLen                         = 1;
input int      InpSlowLen                         = 3;
input int      InpAtrLen                          = 52;
input double   InpStopMult                        = 3.3;
input int      InpNoiseLookback                   = 25;
input int      InpMaxNoCross                      = 4;
input string   InpEntryMode                       = "crossover_plus_pullback";
...
input int      InpTimeDecayBars                   = 10;
input double   InpTimeDecayMinMfeR                = 0.05;
input bool     InpHybridTimeDecayTriageEnabled    = true;
input int      InpHybridTimeDecayCheckpoint       = 30;
input double   InpHybridTimeDecayMaxUnrealizedR   = -1.0;
input double   InpHybridTimeDecayMaxMfeR          = 0.0;
input bool     InpHybridReverseExitTriageEnabled  = true;
input double   InpHybridReverseExitMinMfeR        = 0.0;
input bool     InpShortQualityGateEnabled         = false;
input int      InpShortQualityGateLenBars         = 4800;
input bool     InpTimeRiskFilterEnabled           = false;
...
input bool     InpSkipBelowMinLot                 = true;
...
bool EntryModeAllowsPullback()
{
   return StringCompare(InpEntryMode, "crossover_plus_pullback", false) == 0;
}
...
double RiskLots(const double entryPrice, const double stopPrice)
{
   ...
   if(InpSkipBelowMinLot && lots < minLot)
   {
      DebugLog(StringFormat("ENTRY_SKIPPED_INVALID_LOTS | raw_floor_lots=%.4f | min_lot=%.4f", lots, minLot));
      return 0.0;
   }
...
{
   const int shortQualityRequirement = InpShortQualityGateEnabled ? InpShortQualityGateLenBars + 5 : 0;
   const int requiredBars = MathMax(MathMax(shortQualityRequirement, InpSlowLen + 5),
   ...
   const bool longCrossover = fastPrev <= slowPrev && fastNow > slowNow;
   const bool shortCrossover = fastPrev >= slowPrev && fastNow < slowNow;
   const bool longPullback = EntryModeAllowsPullback() && fastNow > slowNow && closedBar.close < fastNow && closedBar.close > slowNow;
   const bool shortPullback = EntryModeAllowsPullback() && fastNow < slowNow && closedBar.close > fastNow && closedBar.close < slowNow;
   bool longSignal = InpAllowLong && crossCountOk && (longCrossover || longPullback);
   bool shortSignal = InpAllowShort && crossCountOk && (shortCrossover || shortPullback);
