...
- TgSignalSniper management research is a separate offline execution domain: import only finalized WAL-free snapshots, preserve immutable real entries and BID/ASK tick order, and use `tg_signal_management_v1` solely for post-fill policy replay. Never add MT5, Telegram, AutoKraken, demo, live, or automatic-promotion connectors to this domain. Promotion evidence is operation-counted, chronological, exact-coverage, baseline-parity-gated, and decided independently per asset.
