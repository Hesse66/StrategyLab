...
- TgSignalSniper management research is a separate offline execution domain: import only finalized WAL-free snapshots, preserve immutable real entries and BID/ASK tick order, and use `tg_signal_management_v1` solely for post-fill policy replay. Never add MT5, Telegram, AutoKraken, demo, live, or automatic-promotion connectors to this domain. Use exact broker deals including all available costs as the unchanged-parent baseline and exact ticks for counterfactual candidates. Candidate paths may continue past the real close only through a complete declared horizon ending at original TP3, original SL, an applicable forced close, or 48 market hours; incomplete or still-open outcomes are censored. Evaluate each asset only on its most recently published immutable version set and retain older sets as explicit exclusions. Promotion evidence is operation-counted, chronological, exact-coverage, baseline-parity-gated, and decided independently per asset.
