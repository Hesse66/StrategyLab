...
- TgSignalSniper management research is a separate offline execution domain: import only finalized WAL-free snapshots, preserve immutable real entries and BID/ASK tick order, and use `tg_signal_management_v1` solely for post-fill policy replay. Never add MT5, Telegram, AutoKraken, demo, live, or automatic-promotion connectors to this domain. Every closed Axi cohort trade published from 2026-08-10 onward is mandatory per-asset evidence: use broker deals or the persisted broker aggregate for actual performance, retain immutable version sets as diagnostic strata rather than exclusion filters, and never drop a trade merely because its historical version differs. Use exact ticks for promotion-grade counterfactual candidates; when exact ticks are unavailable, retain the imported one-second broker path as explicitly approximate research evidence and never promote from it. Candidate paths may continue past the real close only through a complete declared horizon ending at original TP3, original SL, an applicable forced close, or 48 market hours; incomplete or still-open outcomes are censored but remain listed. Promotion evidence is operation-counted, chronological, exact-coverage, baseline-parity-gated, version-strata-consistent, and decided independently per asset.
