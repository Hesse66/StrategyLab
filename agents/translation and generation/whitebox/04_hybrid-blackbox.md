...
Additional execution constraints: no model feature, label, or veto decision may depend on same-bar information unavailable when the order would be sent. No hybrid mutation may be promoted from a research fill model, realized-only equity curve, or full-sample optimized report alone.
...
Seventh, define the validation contract. This must be chronological, strictly out-of-sample, and explicitly compared against the frozen white-box parent. The hybrid layer does not survive by deleting almost all trades. The validation must also respect execution timing. If the parent decides from a closed candle, the hybrid layer may use that closed candle only after it exists, and its order effect must occur at the next executable price unless the order was already resting. A hybrid model that looks profitable only because it sees the full entry or exit bar before deciding is invalid.
