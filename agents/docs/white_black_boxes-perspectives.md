...
Mutation Lab now treats the white-box, hybrid, and black-box perspectives as a staged evidence process rather than as interchangeable strategy styles. The current production-comparable research route is: preserve or discover an inspectable baseline, optionally transform that baseline toward a desired explanation through phase 1.5 one tested identity layer at a time, optimize it under the MT5-style execution proxy, add explainable phase-3 rule mutations only when diagnostics localize a weakness, add phase-4 hybrid controls only when the parent is already alive, use phase 4.2 black-box viability scans only to discover stable decision-time pockets that can later be converted into explicit live-engine parameters, and use optional phase 4.3 successor-baseline reseeding when a finished lineage is informative but structurally capped.
