...
The MA cross/ATR engine now supports three execution modes. `research_same_close` is diagnostic only: it detects a completed-bar signal and fills at that same close, which is useful for comparing against old research but not for production claims. `next_bar_open` is the default production-comparable bar proxy: it detects signals on the completed bar, creates a pending order, fills at the next bar open with slippage, and allows protective stops after that fill. `mt5_bar_proxy` follows the fork's MetaTrader-style bar simulator more closely: the signal still waits for the next bar open, the entry price is still open plus slippage, but the newly created stop is eligible inside that same entry bar and breakeven stop modifications are rejected when the requested stop would be invalid relative to the current close.

For the current MA/ATR family, `mt5_bar_proxy` is the most realistic bar-level mode available inside Mutation Lab. It is not tick-accurate MT5 and it does not replace live order reconciliation, but it is stricter than same-close research because it models pending entry timing, entry-bar stop risk, stop-modification validity, mark-to-market equity, explicit slippage, and the difference between a strategy signal and an executable order. Use it when judging whether a strategy still has edge under a more broker-like execution path.
...
The current `mt5_bar_proxy` implementation is a step toward that ledger, not the final ledger. It records pending entry orders, pending reverse orders, fills, same-entry-bar stop outcomes, breakeven stop moves, rejected MT5-style stop modifications, exposure-gate blocks, time-decay confirmation decisions, reverse-exit confirmation decisions, and exit-reason metrics. That makes the backtest closer to a real broker simulator while still remaining deterministic and lightweight enough for fast mutation work.

The forked Gann HiLo / Donchian breakout engine has been merged as `ghl_dc_breakout_v1` instead of replacing the existing families. This keeps Mutation Lab's current MA, ASM, and BOS engines intact while adding the fork's breakout route as an explicit `engine_id`. The GHL/DC route uses completed-bar signals, pending next-open or MT5-style entry timing, Donchian breakout confirmation, Gann HiLo trend state, ATR stops, optional breakeven management, mark-to-market equity, and the same production sizing/exposure contract as the rest of the engine.

The forked MT5 fixed-lot risk model has been merged as the `mt5_fixed_risk_lot` sizing mode. It converts a risk-percent budget into broker-style lots using `contract_size`, `min_lot`, `lot_step`, `max_lot`, and `skip_below_min_lot`, then reports invalid-lot skips in diagnostics. This is not a full MT5 terminal simulator, but it prevents the optimizer from treating impossible sub-lot orders as executable and makes account-size feasibility visible before paper trading.
