...
## Mutation Lab Production Addendum

Mutation Lab now uses this canon through a stricter production-research lens. Codification quality is not only whether a rule or model can be written down; it is whether the resulting decision can become an executable order path under realistic assumptions. A production-comparable candidate uses completed-bar signal timing, next executable fills or a stricter broker proxy, mark-to-market equity, explicit costs, production sizing, exposure/risk limits, chronological validation, and exchange-feasibility review before paper-trading claims.

The practical phase rule is affirmative. Phase 1 preserves or discovers an inspectable baseline. Phase 1.5 may transform that baseline toward a desired explanation one tested identity layer at a time, using the baseline as the executable anchor and the explanation as the target identity. Phase 2 tests whether that baseline has broad-history life under realistic execution. Phase 3 adds every explainable rule-family mutation that independently survives one-by-one previews. Phase 4 adds every narrow hybrid mutation that independently survives live-engine previews. Phase 4.2 may scan trade or checkpoint rows for black-box viability, but it promotes nothing directly; it only produces decision-time-safe candidate pockets for future live-engine implementation. Phase 4.3 may use a finished but structurally capped lineage to search for a stronger successor baseline; the successor re-enters phase 2 and must earn every later gate again.

Black-box research is useful when it discovers stable pockets that explicit diagnostics did not reveal. It is not useful when it searches many buckets until one looks good, improves only the full sample, deletes most trades, uses future outcomes as features, or cannot be represented as legal order-time behavior. In Mutation Lab, the white-box parent remains the strategy until a black-box discovery is converted into explicit engine parameters and survives the same production-comparable tests as every other mutation.
