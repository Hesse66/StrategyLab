...
The practical phase rule is affirmative. Phase 1 preserves or discovers an inspectable baseline. Phase 2 tests whether that baseline has broad-history life under realistic execution. Phase 3 adds every explainable rule-family mutation that independently survives one-by-one previews. Phase 4 adds every narrow hybrid mutation that independently survives live-engine previews. Phase 4.2 may scan trade or checkpoint rows for black-box viability, but it promotes nothing directly; it only produces decision-time-safe candidate pockets for future live-engine implementation.
